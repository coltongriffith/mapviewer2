...
export default function LayerList({ layers, selectedLayerId, onSelect, onToggleVisible, onRemove }) {
  return (
    <div className="layer-list">
      {layers.map((layer) => {
        const swatchColor = layer.style?.markerFill || layer.style?.fill || null;
        return (
          <button
            type="button"
            key={layer.id}
          ...
          >
            {swatchColor ? (
              <span className="layer-color-swatch" style={{ background: swatchColor, borderColor: layer.style?.markerColor || layer.style?.stroke || swatchColor }} />
            ) : null}
