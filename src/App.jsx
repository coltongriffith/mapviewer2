...
import {
  buildLegendItems,
  resolveTemplateZones,
} from "./templates/technicalResultsTemplate";
    ...
    const update = () => {
      try {
        ...
        setState({
          ...
          width: Math.max(
            70,
            Math.min(180, Math.round((130 * nice) / Math.max(meters, 1))),
          ),
        });
      } catch {
        // noop
...
function applyModeToProject(project, template, mode) {
  ...
  return {
    ...
    layers: project.layers.map((layer) => ({
      ...layer,
      visible: layer.userStyled
        ? layer.visible
        : preset.visibleRoles
          ? preset.visibleRoles.includes(layer.role) ||
            layer.role === "drillholes"
          : layer.visible,
    })),
    layout: {
      ...project.layout,
      ...
      insetMode:
        project.layout.insetMode === "custom_image"
          ? project.layout.insetMode
          : preset.insetMode || project.layout.insetMode,
...
export default function App() {
  ...
  const [uploadStatus, setUploadStatus] = useState({
    type: "info",
    message:
      "Open the editor, then upload your first file from the left panel.",
  });
  ...
  const selectedCallout = useMemo(
    () =>
      project.callouts.find((callout) => callout.id === selectedCalloutId) ||
      null,
    [project.callouts, selectedCalloutId],
  );
  const selectedMarker = useMemo(
    () =>
      project.markers?.find((marker) => marker.id === selectedMarkerId) || null,
    [project.markers, selectedMarkerId],
  );
  const selectedEllipse = useMemo(
    () =>
      project.ellipses?.find((ellipse) => ellipse.id === selectedEllipseId) ||
      null,
    [project.ellipses, selectedEllipseId],
  );
  ...
  const legendGroups = useMemo(
    () => renderLegendGroups(legendItems, project.layout),
    [legendItems, project.layout],
  );
  const themeTokens = useMemo(
    () => getThemeTokens(project.layout?.themeId || "modern_rounded"),
    [project.layout?.themeId],
  );
  ...
  useEffect(() => {
    ...
    fitProjectToTemplate(
      ...
      project.layout.compositionPreset ||
        template.modePresets?.[project.layout.mode]?.framing ||
        "balanced",
    );
    ...
    template,
    resolvedZones,
    project.layout.frameVersion,
    project.layout.primaryLayerId,
    project.layout.compositionPreset,
    project.layout.zoomPercent,
    project.layers,
  ]);
  ...
  const updateLayout = (patch) => {
    setProject((prev) => ({
      ...
      layout: {
        ...
        legendItems,
        fonts: patch.fonts
          ? { ...(prev.layout.fonts || {}), ...patch.fonts }
          : prev.layout.fonts,
        referenceOverlays: patch.referenceOverlays
          ? {
              ...(prev.layout.referenceOverlays || {}),
              ...patch.referenceOverlays,
            }
          : prev.layout.referenceOverlays,
  ...
  const updateLayer = (layerId, patch) => {
    setProject((prev) => ({
      ...
      layers: prev.layers.map((layer) =>
        layer.id === layerId
          ? { ...mergeDeep(layer, patch), userStyled: true }
          : layer,
      ),
    }));
  };
  ...
  const addGeoJSONLayer = async (file) => {
    ...
    setUploadStatus({
      type: "success",
      message: `Imported ${file.name}. ${kind === "points" ? "Point layer detected and kept visible for editing." : "Layer added successfully."}`,
    });
  };
  ...
  const handleLogoChange = async (e) => {
    ...
    try {
      ...
      setUploadStatus({
        type: "success",
        message: `Loaded logo: ${file.name}`,
      });
    } catch (err) {
      setUploadStatus({
        type: "error",
        message: `Logo import failed: ${err.message}`,
      });
    } finally {
      e.target.value = "";
    }
  ...
  const handleInsetImageChange = async (e) => {
    ...
    try {
      const dataUrl = await readFileAsDataURL(file);
      const insetImageAspect = await readImageAspect(dataUrl);
      updateLayout({
        insetImage: dataUrl,
        insetImageAspect,
        insetMode: "custom_image",
        insetEnabled: true,
      });
      setUploadStatus({
        type: "success",
        message: `Loaded inset image: ${file.name}`,
      });
    } catch (err) {
      setUploadStatus({
        type: "error",
        message: `Inset import failed: ${err.message}`,
      });
    } finally {
      e.target.value = "";
    }
  ...
  const changeLayerRole = (layerId, role) => {
    setProject((prev) => ({
      ...
      layers: prev.layers.map((layer) => {
        ...
        const displayName = cleanLayerName(
  ...
  const improveMap = () => {
    ...
    setUploadStatus({
      type: "success",
      message: "Applied cleaner spacing and layout defaults.",
    });
  };

  const addCalloutAtAnchor = ({
    text,
    subtext = "",
    type = "leader",
    anchor,
  ...
  const updateCallout = (calloutId, patch) => {
    setProject((prev) => ({
      ...
      callouts: prev.callouts.map((callout) =>
        callout.id === calloutId ? { ...callout, ...patch } : callout,
      ),
    }));
  };

  const updateCalloutAnchor = (calloutId, anchor) => {
    setProject((prev) => ({
      ...prev,
      callouts: prev.callouts.map((callout) =>
        callout.id === calloutId ? { ...callout, anchor } : callout,
      ),
    }));
  };
  ...
  const removeCallout = (calloutId) => {
    setProject((prev) => ({
      ...prev,
      callouts: prev.callouts.filter((callout) => callout.id !== calloutId),
    }));
  ...
  const handleFeatureClick = ({ layerId, feature, latlng }) => {
    ...
    setSelectedFeature({
      ...
      featureId:
        ...
        `${layerId}:${latlng.lat.toFixed(6)}:${latlng.lng.toFixed(6)}`,
  ...
  const mapZoomPercent = Math.max(
    10,
    Math.min(400, Number(project.layout.zoomPercent || 100)),
  );
  const featureEditorPoint = useMemo(() => {
    if (!leafletMapRef.current || !selectedFeature?.latlng) return null;
    const pt = leafletMapRef.current.latLngToContainerPoint([
      selectedFeature.latlng.lat,
      selectedFeature.latlng.lng,
    ]);
  ...
  const updateEllipse = (ellipseId, patch) => {
    setProject((prev) => ({
      ...prev,
      ellipses: (prev.ellipses || []).map((ellipse) =>
        ellipse.id === ellipseId ? { ...ellipse, ...patch } : ellipse,
      ),
    }));
  };
  ...
  const removeEllipse = (ellipseId) => {
    setProject((prev) => ({
      ...prev,
      ellipses: (prev.ellipses || []).filter(
        (ellipse) => ellipse.id !== ellipseId,
      ),
    }));
  ...
  const handleExport = async (format) => {
    ...
    try {
      const scene = buildScene(
        ...
        { ...project, layout: { ...project.layout, legendItems } },
      ...
      <div className="map-topbar">
        <div className="map-topbar-left">
          <div className="map-topbar-title">
            {project.layout.title || "Project Map"}
          </div>
          <div className="map-topbar-sub">
            {annotationTool
              ? `Mode: ${annotationTool === "marker" ? "Add Marker" : "Add Zone"} (click map to place)`
              : "Pan map, add markers/zones, and export."}
          </div>
        </div>
        <div className="map-topbar-right">
          ...
          <button
            ...
            onClick={() => {
              setAnnotationTool(
                annotationTool === "ellipse" ? null : "ellipse",
              );
              setSelectedFeature(null);
            }}
          >
          ...
          <button className="btn" type="button" onClick={autoFrameAll}>
            Refit
          </button>
          <button
            className="btn"
      ...
      <div className="app-shell">
        <Sidebar>
          <div className="sidebar-header-row">
            <div>
              ...
              <p className="sidebar-subtitle">
                Upload on the left, design in the center, export when ready.
              </p>
            </div>
            <button
            ...
            <div className="control-grid">
              ...
              <div className="control-row inline-2">
                <div>
                  ...
                  <select
                    value={project.layout.themeId || "modern_rounded"}
                    onChange={(e) => updateLayout({ themeId: e.target.value })}
                  >
                ...
                <div>
                  ...
                  <select
                    value={project.layout.compositionPreset}
                    onChange={(e) =>
                      updateLayout({
                        compositionPreset: e.target.value,
                        frameVersion: (project.layout.frameVersion || 0) + 1,
                      })
                    }
                  >
                    {Object.entries(COMPOSITION_PRESETS).map(
                      ([value, label]) => (
                        <option key={value} value={value}>
                          {label}
                        </option>
            ...
            <div className="control-grid">
              ...
              <div className="control-row inline-2">
                ...
                <div>
                  ...
                  <select
                    value={project.layout.insetMode}
                    onChange={(e) =>
                      updateLayout({
                        insetMode: e.target.value,
                        insetEnabled: true,
                      })
                    }
                  >
              ...
              <div className="small-note">
                Zoom is controlled directly on the map toolbar (10% to 400%) for
                fine framing control.
              </div>
              <div className="control-row inline-2">
                <div>
                  ...
                  <select
                    value={selectValue(
                      FONT_OPTIONS,
                      project.layout.fonts?.title,
                    )}
                    onChange={(e) =>
                      updateLayout({ fonts: { title: e.target.value } })
                    }
                  >
                ...
                <div>
                  ...
                  <select
                    value={selectValue(
                      FONT_OPTIONS,
                      project.layout.fonts?.legend,
                    )}
                    onChange={(e) =>
                      updateLayout({ fonts: { legend: e.target.value } })
                    }
                  >
                    {Object.entries(FONT_OPTIONS).map(([value, label]) => (
                      <option key={value} value={value}>
              ...
              <div className="control-row inline-2">
                <div>
                  ...
                  <select
                    value={selectValue(
                      FONT_OPTIONS,
                      project.layout.fonts?.label,
                    )}
                    onChange={(e) =>
                      updateLayout({ fonts: { label: e.target.value } })
                    }
                  >
                    {Object.entries(FONT_OPTIONS).map(([value, label]) => (
                      <option key={value} value={value}>
                ...
                <div>
                  ...
                  <select
                    value={selectValue(
                      FONT_OPTIONS,
                      project.layout.fonts?.callout,
                    )}
                    onChange={(e) =>
                      updateLayout({ fonts: { callout: e.target.value } })
                    }
                  >
                    {Object.entries(FONT_OPTIONS).map(([value, label]) => (
                      <option key={value} value={value}>
              ...
              <div className="button-row three">
                ...
                <button
                  className="btn"
                  type="button"
                  onClick={() => insetInputRef.current?.click()}
                >
                  Upload Inset
                </button>
                <button
                  ...
                  onClick={() =>
                    updateLayout({
                      insetEnabled: project.layout.insetEnabled === false,
                    })
                  }
                >
                  {project.layout.insetEnabled === false
                    ? "Show Inset"
                    : "Hide Inset"}
                </button>
              </div>
              <div className="control-row inline-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateLayout({ logoScale: Number(e.target.value) })
                    }
                  />
                </div>
                <div className="range-value">
                  {Math.round((project.layout.logoScale || 1) * 100)}%
                </div>
              </div>
              <div className="control-row inline-2">
                <div>
                  ...
                  <input
                    ...
                    max="1.6"
                    step="0.05"
                    value={project.layout.insetScale || 1}
                    onChange={(e) =>
                      updateLayout({ insetScale: Number(e.target.value) })
                    }
                  />
                </div>
                <div className="range-value">
                  {Math.round((project.layout.insetScale || 1) * 100)}%
                </div>
              </div>
              <div className="small-note">
                Recommended inset size: roughly 15–25% of the map width. The
                inset now scales with the uploaded image ratio to reduce
                cropping.
              </div>
              <input
                ref={logoInputRef}
              ...
              <input
                ...
                type="file"
                accept="image/*"
                onChange={handleInsetImageChange}
                hidden
              />
              {project.layout.insetMode === "custom_image" ? (
                  ...
                  {project.layout.insetImage ? (
                    <>
                      <div className="inset-preview">
                        <img
                          src={project.layout.insetImage}
                          alt="Inset preview"
                        />
                      </div>
                      <div className="small-note">
                        Custom inset is active and anchored in the top-right
                        corner.
                      </div>
                      <button
                        className="secondary-btn"
                        type="button"
                        onClick={() => updateLayout({ insetImage: null })}
                      >
                        Remove Inset Image
                      </button>
                    </>
                    ...
                    <div className="small-note">
                      Custom inset mode is selected, but no image is loaded yet.
                    </div>
                  )}
                </div>
              ) : null}
          ...
          <section className="control-section">
            ...
            <div className="toggle-grid">
              <label className="toggle-row">
                <input
                  type="checkbox"
                  checked={referenceOverlays.context}
                  onChange={(e) =>
                    updateLayout({
                      referenceOverlays: { context: e.target.checked },
                    })
                  }
                />{" "}
                <span>Roads / Water / Towns</span>
              </label>
              <label className="toggle-row">
                <input
                  type="checkbox"
                  checked={referenceOverlays.labels}
                  onChange={(e) =>
                    updateLayout({
                      referenceOverlays: { labels: e.target.checked },
                    })
                  }
                />{" "}
              ...
              <label className="toggle-row">
                <input
                  type="checkbox"
                  checked={referenceOverlays.rail}
                  onChange={(e) =>
                    updateLayout({
                      referenceOverlays: { rail: e.target.checked },
                    })
                  }
                />{" "}
                <span>Railways</span>
          ...
          <section className="control-section">
            ...
            {selectedLayer ? (
              <div className="control-grid" style={{ marginTop: 10 }}>
                <div className="control-row">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDisplayLabel(selectedLayer.id, e.target.value)
                    }
                  />
                </div>
                <div className="control-row">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      changeLayerRole(selectedLayer.id, e.target.value)
                    }
                  >
                ...
                <div className="button-row three">
                  ...
                  >
                    {project.layout.primaryLayerId === selectedLayer.id
                      ? "Framing Layer"
                      : "Use for Framing"}
                  </button>
                  <button
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={
                        ...
                        selectedLayer.style?.markerColor ||
                        "#2563eb"
                      }
                      onChange={(e) =>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={
                        ...
                        selectedLayer.style?.markerFill ||
                        "#93c5fd"
                      }
                      onChange={(e) =>
                        updateLayer(selectedLayer.id, {
                ...
                {isPointStyledLayer(selectedLayer) ? (
                  <div className="control-row inline-2">
                    ...
                    <div className="range-value">
                      {selectedLayer.style?.markerSize ?? 12}px
                    </div>
                  </div>
                ) : (
                  <div className="control-row inline-2">
                    ...
                    <div className="range-value">
                      {Math.round(
                        (selectedLayer.style?.fillOpacity ?? 0.22) * 100,
                      )}
                      %
                    </div>
                  </div>
              ...
              <p className="small-note">
                Select a layer to edit its display label, role, order, and
                colors.
              </p>
            )}
          </section>
          ...
          <section className="control-section">
            ...
            <div className="button-row">
              <button
                className={`secondary-btn ${annotationTool === "marker" ? "active-toggle" : ""}`}
                type="button"
                onClick={() => {
                  setAnnotationTool(
                    annotationTool === "marker" ? null : "marker",
                  );
                  setSelectedFeature(null);
                }}
              >
              ...
              <button
                className={`secondary-btn ${annotationTool === "ellipse" ? "active-toggle" : ""}`}
                type="button"
                onClick={() => {
                  setAnnotationTool(
                    annotationTool === "ellipse" ? null : "ellipse",
            ...
            <div className="small-note" style={{ marginTop: 8 }}>
              {annotationTool
                ? "Click anywhere on the map to place the selected annotation."
                : "Add highlight markers or dashed ellipses anywhere on the map."}
            </div>
            ...
            {selectedEllipse ? (
              <div className="control-grid" style={{ marginTop: 10 }}>
                ...
                <div className="control-row">
                  <label>Label</label>
                  <input
                    value={selectedEllipse.label || ""}
                    onChange={(e) =>
                      updateEllipse(selectedEllipse.id, {
                ...
                <div className="control-row inline-2">
                  <div>
                    ...
                    <input
                      ...
                      max="180"
                      step="1"
                      value={selectedEllipse.rotation}
                      onChange={(e) =>
                        updateEllipse(selectedEllipse.id, {
                ...
                <label className="toggle-row">
                  <input
                    type="checkbox"
                    checked={selectedEllipse.dashed !== false}
                    onChange={(e) =>
                      updateEllipse(selectedEllipse.id, {
                ...
                <button
                  className="secondary-btn"
                  type="button"
                  onClick={() => removeEllipse(selectedEllipse.id)}
                >
            ...
            {selectedFeature ? (
              <div className="control-grid">
                <div className="feature-chip">
                  Selected: {selectedFeature.layerName}
                </div>
                <div className="small-note">
                  Click a drillhole on the map, then refine the callout here.
                  The selected hole is editable before you add the callout.
                </div>
                <div className="control-row">
                  <label>Title</label>
                  <input
                ...
                <div className="control-row">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSelectedFeature((prev) => ({
                        ...prev,
                        suggestedSubtext: e.target.value,
                      }))
                    }
                  />
                </div>
                <div className="control-row inline-2">
                  <div>
                    ...
                    <select
                      value={selectedFeature.calloutType || "leader"}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      max="320"
                      step="2"
                      value={selectedFeature.boxWidth || 188}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                          boxWidth: Number(e.target.value || 188),
                        }))
                      }
                    />
                  </div>
                ...
                <div className="control-row inline-2">
                  <div>
                    ...
                    <input
                      type="color"
                      value={selectedFeature.style?.background || "#ffffff"}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                          style: {
                  ...
                  <div>
                    ...
                    <input
                      type="color"
                      value={selectedFeature.style?.border || "#102640"}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                          style: {
                ...
                <div className="control-row inline-2">
                  <div>
                    ...
                    <input
                      type="color"
                      value={selectedFeature.style?.textColor || "#0f172a"}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                          style: {
                  ...
                  <div>
                    ...
                    <input
                      type="color"
                      value={selectedFeature.style?.subtextColor || "#475569"}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                          style: {
                ...
                <div className="control-row inline-2">
                  <div>
                    ...
                    <input
                      ...
                      max="16"
                      step="1"
                      value={selectedFeature.style?.fontSize || 12}
                      onChange={(e) =>
                        setSelectedFeature((prev) => ({
                          ...prev,
                          style: {
                  ...
                  <div className="range-value">
                    {selectedFeature.style?.fontSize || 12}px
                  </div>
                </div>
                <button
                  className="btn primary"
                  type="button"
                  onClick={addCalloutFromSelectedFeature}
                >
                  Add / Update Callout
                </button>
              </div>
              ...
              <div className="small-note">
                Click a drillhole point on the map to open its callout editor.
              </div>
            )}
          </section>
          ...
          <section className="control-section">
            ...
            <div className="button-row" style={{ marginBottom: 10 }}>
              <button
                className="btn primary"
                type="button"
                onClick={addCalloutFromSelectedLayer}
                disabled={!selectedLayer}
              >
                Add From Selected Layer
              </button>
              <button className="btn" type="button" onClick={autoFrameAll}>
                Auto Frame All
              </button>
            ...
            {selectedCallout ? (
              <div className="selected-note">
                Selected callout: {selectedCallout.text}
              </div>
            ) : null}
            <div className="callout-list">
              {project.callouts.map((callout, index) => (
                <div
                  key={callout.id}
                  className={`callout-card ${selectedCalloutId === callout.id ? "active" : ""}`}
                >
                  ...
                  <div className="control-grid">
                    <div className="control-row">
                      ...
                      <input
                        value={callout.text}
                        onChange={(e) =>
                          updateCallout(callout.id, { text: e.target.value })
                        }
                      />
                    </div>
                    <div className="control-row inline-2">
                      <div>
                        ...
                        <select
                          value={callout.type}
                          onChange={(e) =>
                            updateCallout(callout.id, { type: e.target.value })
                          }
                        >
                          {Object.entries(CALLOUT_TYPES).map(
                            ([value, label]) => (
                              <option key={value} value={value}>
                                {label}
                              </option>
                      ...
                      <div>
                        ...
                        <select
                          value={callout.priority}
                          onChange={(e) =>
                            updateCallout(callout.id, {
                    ...
                    <div className="nudge-grid">
                      ...
                      <button
                        className="secondary-btn"
                        type="button"
                        onClick={() => nudgeCallout(callout.id, 0, -8)}
                      >
                        ↑
                      </button>
                      <span />
                      <button
                        className="secondary-btn"
                        type="button"
                        onClick={() => nudgeCallout(callout.id, -8, 0)}
                      >
                        ←
                      </button>
                      <button
        ...
        <div className="editor-main">
          ...
          >
            ...
            <CalloutsOverlay
              ...
              onMoveAnchor={updateCalloutAnchor}
              fontFamily={project.layout.fonts?.callout}
            />

            <div
              className="template-zone"
              style={zoneStyle(resolvedZones.title)}
            >
            ...
            {legendItems.length ? (
              <div
                className="template-zone"
                style={zoneStyle(resolvedZones.legend)}
              >
                          ...
                          <div key={item.id} className="legend-item">
                            {item.type === "points" ? (
                              <span
                                ...
                                style={{
                                  borderColor: item.style.markerColor || "#111",
                              ...
                              <span
                                ...
                                style={{
                                  ...
                                  background: item.style.fill || "#93c5fd",
                                  opacity: item.style.fillOpacity ?? 1,
                                }}
                              />
                            )}
            ...
            <div className="floating-north-arrow">
              <NorthArrow />
            </div>
            {project.layout.insetEnabled !== false &&
            resolvedZones.inset?.width ? (
              <div
                className="template-zone"
                style={zoneStyle(resolvedZones.inset)}
              >
                <LocatorInset
                  layers={project.layers}
                  insetMode={project.layout.insetMode}
                  insetImage={project.layout.insetImage}
                  insetImageAspect={project.layout.insetImageAspect}
            ...
            <div
              className="template-zone"
              style={zoneStyle(resolvedZones.scaleBar)}
            >
              <ScaleBar map={leafletMapRef.current} />
            </div>
            {project.layout.footerText &&
            project.layout.footerEnabled !== false ? (
              <div
                className="template-zone"
                style={zoneStyle(resolvedZones.footer)}
              >
                <div className="template-card footer-card">
                  {project.layout.footerText}
                </div>
              </div>
            ) : null}
            {project.layout.logo ? (
              <div
                className="template-zone"
                style={zoneStyle(resolvedZones.logo)}
              >
                <div className="template-card logo-card">
                  <img src={project.layout.logo} alt="Logo" />
                </div>
              </div>
            ) : null}
            {selectedFeature && featureEditorPoint ? (
              <div
                className="drillhole-inline-editor"
                style={{
                  left: featureEditorPoint.left,
                  top: featureEditorPoint.top,
                }}
              >
                <div className="drillhole-inline-title">
                  Edit drillhole callout
                </div>
                <input
                  value={selectedFeature.suggestedLabel}
                  onChange={(e) =>
