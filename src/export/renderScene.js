...
function drawNorthArrowCanvas(ctx, scene, scale) {
  ...
  ctx.fillStyle = '#122033'; ctx.font = `700 ${14 * scale}px Arial`; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText('N', cx, y + 12 * scale);
...
function drawScaleBarCanvas(ctx, scene, scale) {
  ...
  drawRoundedRect(ctx, x, y, w, h, 10 * scale); ctx.fillStyle = 'rgba(255,255,255,0.95)'; ctx.fill(); ctx.strokeStyle = 'rgba(18,30,48,0.18)'; ctx.stroke();
  ctx.fillStyle = '#122033'; ctx.fillRect(x + 16 * scale, y + 18 * scale, barWidth / 2, 10 * scale); ctx.fillStyle = '#ffffff'; ctx.fillRect(x + 16 * scale + barWidth / 2, y + 18 * scale, barWidth / 2, 10 * scale); ctx.strokeStyle = '#122033'; ctx.lineWidth = Math.max(1, scale); ctx.strokeRect(x + 16 * scale, y + 18 * scale, barWidth, 10 * scale);
  ctx.fillStyle = '#1d2b3d'; ctx.font = `${12 * scale}px Arial`; ctx.textBaseline = 'top'; ctx.fillText(scaleState.label, x + 16 * scale, y + 40 * scale);
}
...
function drawCalloutsCanvas(ctx, scene, scale) {
  placeCallouts(scene, scale).forEach((c) => {
    ...
    ctx.setLineDash([]);
    if (c.type !== 'plain') { drawRoundedRect(ctx, c.left, c.top, c.width, c.height, 6 * scale); ctx.fillStyle = 'rgba(255,255,255,0.97)'; ctx.fill(); ctx.strokeStyle = '#17304f'; ctx.lineWidth = 1 * scale; ctx.stroke(); }
    ctx.fillStyle = '#102640'; ctx.font = `700 ${12 * scale}px Arial`; ctx.textBaseline = 'middle'; ctx.fillText(c.text || '', c.left + (c.type === 'plain' ? 0 : 10 * scale), c.top + (c.type === 'plain' ? 10 * scale : c.height / 2));
  });
}
