-- Landing-page outcomes share the existing admin-only report and session exclusions.
...
begin
  ...
  pv as materialized (
    select e.session_id, e.created_at, e.utm_source, e.utm_medium, e.referrer, e.path
  ...
  first_page as (
    select distinct on (session_id) session_id, public.em_source_label(utm_source, utm_medium, referrer) as source,
      nullif(split_part(split_part(path, '?', 1), '#', 1), '') as landing_path
  ...
  sessions as materialized (
    select s.session_id, coalesce(f.source, 'Unattributed') source, coalesce(f.landing_path, '(unrecorded)') landing_path,
      exists (select 1 from pe e where e.session_id = s.session_id and (e.event = 'registry_claims_imported' or (e.event = 'layer_added' and e.props->>'source' in ('upload','csv','registry')))) as deliberate_data,
      exists (select 1 from pe e where e.session_id = s.session_id and e.event = 'layer_added' and e.props->>'source' = 'deeplink') as company_preview,
      exists (select 1 from pe e where e.session_id = s.session_id and e.event = 'layer_added' and e.props->>'source' = 'demo') as demo,
      exists (select 1 from pe e where e.session_id = s.session_id and e.event = 'project_saved') as saved,
      exists (select 1 from public.leads l where l.session_id = s.session_id and l.captured_at >= p_start and l.captured_at < p_end) as lead,
  ...
  select jsonb_build_object(
    ...
    'landing_pages', (select coalesce(jsonb_agg(t order by t.real_exports desc, t.deliberate_data desc, t.opened desc, t.sessions desc, t.path), '[]'::jsonb) from (
      select landing_path as path, count(*) as sessions, count(opened_at) as opened,
        count(*) filter (where deliberate_data) as deliberate_data,
        count(*) filter (where company_preview) as company_previews,
        count(*) filter (where demo) as demos,
        count(*) filter (where real_exported) as real_exports,
        count(*) filter (where saved) as saved_sessions,
        count(*) filter (where lead) as lead_sessions
      from sessions group by landing_path
      order by real_exports desc, deliberate_data desc, opened desc, sessions desc, path limit 50) t),
