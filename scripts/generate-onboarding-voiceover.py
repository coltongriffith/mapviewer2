...
cues = json.loads((ROOT / "narration-cues.json").read_text(encoding="utf-8"))
...
for index, cue in enumerate(cues, start=1):
    chunks = []
    for _, _, audio in pipeline(cue["spokenText"], voice=VOICE, speed=SPEED, split_pattern=r"\n+"):
    ...
    print(f"Cue {index:02d}: {cue['atMs']/1000:.2f}s, {cue_audio.size/SAMPLE_RATE:.2f}s — {cue['spokenText']}")
...
for index, ((cue, audio), next_item) in enumerate(zip(rendered, rendered[1:]), start=1):
    ...
    if cue_end_ms > next_start_ms - 80:
        raise RuntimeError(f"Cue {index} ends at {cue_end_ms}ms, too close to cue {index + 1} at {next_start_ms}ms")

total_samples = max(round(cue["atMs"] * SAMPLE_RATE / 1000) + audio.size for cue, audio in rendered) + SAMPLE_RATE
...
if peak > 0.94:
    voiceover *= 0.94 / peak
sf.write(ROOT / "voiceover-natural.wav", voiceover, SAMPLE_RATE, subtype="PCM_16")
print(f"Saved {ROOT / 'voiceover-natural.wav'}")
