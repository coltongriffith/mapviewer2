...
test('the hero image is a right-sized WebP, not a multi-megabyte PNG', async ({ page }) => {
  const heroAssets = [];
  page.on('response', (r) => {
    if (/\/(?:gallery\/hero-exploration|landing\/hero-valley)/.test(r.url())) heroAssets.push(r.url().split('/').pop());
  });
  ...
  expect(heroAssets.some((f) => f === 'hero-exploration.png' || f === 'hero-valley.jpg'), 'a heavy fallback was downloaded as well').toBe(false);
