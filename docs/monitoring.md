...
| User feedback / issue reports | `src/components/FeedbackModal.jsx` → `api/feedback.js` → `public.feedback` (`supabase/migrations/20260910000001_user_feedback.sql`) | "Report a problem / send feedback" in the sidebar account panel (signed in or not) and "Report this problem" on the crash screen. Each report carries the page, release, session id, viewport and the last few client errors from that tab, so a two-word report still arrives with the exception behind it. A trigger emails the owner via Resend (same Vault secrets as the signup email; swallows failures — the row is the record). Triage in Admin → **Feedback** (`admin_get_feedback`, `admin_set_feedback_status`). |
