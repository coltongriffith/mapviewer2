...
| User feedback / issue reports | `src/components/FeedbackModal.jsx` → `api/feedback.js` → `public.feedback` (`supabase/migrations/20260910000001_user_feedback.sql`) | "Report a problem / send feedback" in the sidebar account panel (signed in or not) and "Report this problem" on the crash screen. Each report carries the page, release, session id, viewport and the last few client errors from that tab, so a two-word report still arrives with the exception behind it. A trigger emails the Vault secret `feedback_notification_to` via Resend (falls back to `signup_notification_to`, then support@explorationmaps.com; swallows failures — the row is the record). Triage in Admin → **Feedback** (`admin_get_feedback`, `admin_set_feedback_status`). |
| New-signup email | `supabase/migrations/20260729000001_new_signup_notification.sql` | A trigger on `auth.users` emails support@explorationmaps.com via Resend the moment a new account is created. Wrapped in an exception handler so a Resend outage or bad key can never fail a real signup — it just silently skips. The API key lives in Supabase Vault (`select vault.create_secret(...)`), never in a migration file or table. To change the destination address or sender, edit `public.send_signup_notification` (a plain SQL function, no redeploy needed) and re-run it as a migration. |
